...
% Example:
%     dataset = TOF_PROCESS( dataset, ...
%         'NumberOfChannels', num_channels, ...
%         'UseExternalDistance', false, ...
%         'DistanceToSpectrometer', tof_distance(119)*1e-3, ...
%         'MoleculeMassAMU', 28, ...
%         'StartIndex', 20, ...
%         'TolT', 0.0, ...
%         'TolV', 0.0, ...
...
%         'StartV', [1233 303 0]);
%
% dataset is a struct and has to contain the fields 'flux', 'time' and
% 'filename'. dataset.time should be given in seconds. The order of
% magnitude for dataset.flux should be 1e10.
